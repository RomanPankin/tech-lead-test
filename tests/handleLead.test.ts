...
import type { LeadStore, StoredLead, DeliveryStatus } from '@/lib/types';
...
describe('handleLead', () => {
  ...
  it('marks delivered when all adapters succeed (demo mode)', async () => {
    const store = new MemoryStore();
    const result = await handleLead(base, store);
  ...
  it('stamps an id, destination and receivedAt', async () => {
    ...
    const result = await handleLead(base, store, fixed);
